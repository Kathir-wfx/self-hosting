...
let wfOrgId = "";
let jsMetadata = {};
let jsMetadataPollFrequency = 15;
...
let appLocale = "";
let forceSendObserverLogs = false;
const jsVersionNumber = 5089;
let userSessionId;
...
let observerDataSize = {
  numberOfEvents: 0,
  totalSize: 0,
  avgSize: 0,
  keys: {}
};
const webviewScrollThresholds = {
  WIDTH:0,
  HEIGHT:0.1,
  TOP:50,
  LEFT:0,
...
function hasAllValidKeys(project) {
  ...
  // Check if all required keys exist in the project object
  const hasKeys = requiredKeys.every((key) => key in project);

  // Ensure `initialTerminationStatus` and `terminationStatus` are valid objects
  const hasValidObjects =
      typeof project.initialTerminationStatus === "object" &&
...
function noActiveProjects() {
  return (
    ...
    sortedProjectArray?.every(
      (project) => {
        if (allProjectStatus?.[project.id] && !hasAllValidKeys(allProjectStatus?.[project.id])) return false;
...
function findTerminationStatuses(project, savedAllProjectStatus) {
  ...
  let isTerminationStatusObjectExisting = savedAllProjectStatus?.[objKey] && savedAllProjectStatus[objKey]?.terminationStatus;

  let terminationStatus = isTerminationStatusObjectExisting ? savedAllProjectStatus[objKey].terminationStatus
    : {...terminationTemplate};
...
function initialiseConnectedProjectsParentObjectMap(project) {
  try {
    ...
    // Added fallback {} to handle cases that were previously mishandled and caused issues
...
function updateViewStatusForTheProject(activeProjectId, key, traceId) {
  if (activeProjectId !== undefined) {
    const project = allProjectStatus[activeProjectId];
    ...
    if (project?.triggerCase === "PLAY_ONCE")
    ...
    if (key === "nSession" && project.shownOnce === false) {
        ...
        if (terminationStatus.nSession >= initialTerminationStatus.nSession) {
          sendAnalyticEvent(ANALYTICS_EVENTS.PROJECT_TERMINATION, {
            terminationRule: `nSession: ${initialTerminationStatus.nSession}`,
          }, traceId);
        }
...
function reComputeIsFlowTriggered() {
  if (allProjectStatus == undefined) return;

  for (const projectStatus of Object.values(allProjectStatus)) {
    const { triggerCase, terminationStatus } = projectStatus;
    ...
    switch (triggerCase) {
      ...
      case "MANUAL_TRIGGER":
        ...
        break;
      case "PLAY_ONCE":
        projectStatus.isFlowTriggered = terminationStatus?.nSession > 0;
        break;
      case "EVERY_SESSION_UNTIL_DISMISSED":
        projectStatus.isFlowTriggered = terminationStatus?.nDismissedByUser > 0;
        break;
      case "EVERY_SESSION_UNTIL_FLOW_COMPLETE":
        projectStatus.isFlowTriggered = terminationStatus?.perApp > 0;
        break;
      case "EVERY_SESSION_UNTIL_ALL_FLOWS_ARE_COMPLETED":
        projectStatus.isFlowTriggered = !!terminationStatus?.untilAllFlowsAreCompleted;
        break;
      default:
        projectStatus.isFlowTriggered = false;
    }
...
const sdkStateService = {
  init(storedSdkStateString = "{}") {
    ...
    this.profileConfig = sdkState.profileConfig;
...
function filterHierarchy(node, currentPlatform = platform, isLayout = false) {
  ...
  if (isRoot) {
    ...
    const ignoreNode =
      currentPlatform === PLATFORMS.ANDROID
        ? ignoreInvalidViewsAndroid(node,features)
  ...
  if (filteredChildren.length > 1) {
    ...
    !node?.is_long_clickable
      && !(node?.tag && features?.nonInteractiveExclusions?.tags?.includes(node?.tag))
  ) {
    return;
  }
...
function ignoreInvalidViewsAndroid(node,appFeatures={}) {

  if (!metaConfigs?.enableHorizontalAutoScroll &&  (node?.bounds?.right <= 5 || node?.bounds?.left >= screenWidth)){
    return true;
  }

  // Ignore if the visibility is not 0
  if (node?.visibility && node.visibility !== 0) return true;
  ...
  if (
    ...
    return !(node?.tag && appFeatures?.nonInteractiveExclusions?.tags?.includes(node?.tag));
  }
...
function ignoreOverlappingNodesIOS(nodes) {
  ...
  for (let i = 0; i < sortedNodes.length; i++) {
    ...
    // The views behind a non-interactable view are not ignored
    if (features?.ignoreNonInteractableViews && !currentObject.is_user_interaction_enabled) continue;
...
function extractViewIdFromValue({key, value, identifierVersion,packageName}){
  if(identifierVersion==="2") return value;
  switch (key){
    case "ar_id":
      return value?.split(`android:id/`).pop();
    case "id":
      return value?.split(`${packageName}:id/`).pop();
    default:
      return value
  }
}
...
function findMatchingNode(root, identifier, regex, identifierVersion) {
  ...
  const isMatchingNode = (node, identifier) => {
    for (const [key, value] of Object.entries(identifier)) {
      ...
      if (key === "ar_id") {
        const nodeId = extractViewIdFromValue({key,identifierVersion,value:node?.id});
        if (regex?.[key]) {
      ...
      // Special handling for 'id', compare after transformation
        ...
        const nodeId = extractViewIdFromValue({key,identifierVersion, value: node?.id, packageName: root?.ROOT_NODE?.client_package_name});
        if (regex?.[key]) {
          if (!parseRegexString(regex?.[key])?.test(nodeId)) return false;
        } else {
          if (nodeId !== value) return false;
...
function findMatches(filteredHierarchy, identifiers, traceId) {
  ...
  identifiersArray?.forEach((identifier) => {
    ...
    const identifierVersion = identifier.version;
          ...
          for (const key in identifier) {
            ...
            if (regex?.[key] !== undefined) {
              ...
              if (
                ...
                key === "ar_id" &&
                !regexValue.test(extractViewIdFromValue({key,identifierVersion,value:node?.id}))
              ) {
                ...
                key === "id" &&
                  !regexValue.test(
                      extractViewIdFromValue({key,identifierVersion,value:node?.id,packageName:filteredHierarchy?.client_package_name})
                  )
              ) {
              ...
              if (
                ...
                key === "ar_id" &&
                  identifier[key] !== extractViewIdFromValue({key,identifierVersion,value:node?.id})
              ) {
                ObserverData.ar_id = "false";
                isMatch = false; // If any key doesn't match, it's not a match
                break;
                ...
                key === "id" &&
                  identifier[key] !== extractViewIdFromValue({key,identifierVersion,value:node?.id,packageName:filteredHierarchy?.client_package_name})
              ) {
                ObserverData.r_id = "false";
                isMatch = false; // If any key doesn't match, it's not a match
                break;
          ...
          for (const key in target) {
            if (targetRegex?.[key] !== undefined) {
              ...
              if (
                ...
                key === "ar_id" &&
                !regexValue.test(extractViewIdFromValue({key,identifierVersion,value:actualNode?.id}))
              ) {
                ObserverData.ar_id = "false";
                isMatch = false; // If any key doesn't match, it's not a match
                break;
                ...
                key === "id" &&
                !regexValue.test(
                    extractViewIdFromValue({key,identifierVersion,value:actualNode?.id,packageName:filteredHierarchy?.client_package_name})
                )
              ) {
                ObserverData.r_id = "false";
                isMatch = false; // If any key doesn't match, it's not a match
              ...
              if (
                ...
                key === "ar_id" &&
                  target?.[key] !== extractViewIdFromValue(({key,identifierVersion,value:actualNode?.id}))
              ) {
                ObserverData.ar_id = "false";
                isMatch = false; // If any key doesn't match, it's not a match
                break;
                ...
                key === "id" &&
                  target?.[key] !== extractViewIdFromValue(({key,identifierVersion,value:actualNode?.id,packageName:filteredHierarchy?.client_package_name}))
              ) {
                ObserverData.r_id = "false";
                isMatch = false; // If any key doesn't match, it's not a match
                break;
...
function getHierarchyIntervalTime() {
  return (metaConfigs?.screenHierarchy?.intervalTime || features?.hierarchyLoopInterval?.intervalTime || intervalTime);
}
...
function sendWebJSscript({webIdentifiers, webViewNode}, traceId) {
  ...
  //TODO OBSERVER: send number of webviews in heierarchy in case of multiple
...
function flushFinderObserverEvents(traceId) {
    finderObserverEvents.forEach((finderEvent) => observerLogService.sendObserverLog(finderEvent));
}

function extractProjectDataForObserver(project) {
  return project ? {
    project_id: project?.id,
    project_name: project?.projectParameters?.projectName,
    project_type: project?.projectParameters?.projectType,
  } : {};
}
  ...
  function logCommandToNativeLayer(message, traceId) {
    try {
      if (shouldLogCommandName(message)) {
        let messageData = shouldLogData(message) ? message.data ?? {} : {message_data_included: false};
        sendObserverLogs(
          {
          ...
          sendObserverLogs(
            {
              ...
              category: OBSERVER_EVENT_CATEGORIES.FINDER,
              properties: {
                ...extractProjectDataForObserver(activeProject),
                step_type: stepFound.assistType,
                step_name: stepFound.assist.name,
                step_id: stepFound.id,
              },
              level: ObserverLogLevel.DEBUG,
            },
            traceId
          ...
          sendObserverLogs(
            {
              ...
              properties: {
                ...
                step_name: activeStep?.step?.assist?.name,
                step_type: activeStep?.step?.assistType,
                ...extractProjectDataForObserver(activeProject),
              },
              level: ObserverLogLevel.DEBUG,
            },
...
function callWebScriptForAllProject(traceId) {
  const webViewProjects = embedProject
    ...
    .filter((project) => project?.webIdentifiers);
...
function projectHasAppLocale(project) {
  if(!appLocale){
    return true;
  }
  const index = project?.languages?.findIndex((lang)=> lang.localeId === appLocale);
...
function runDetectionLogicForProjects(
  ...
  const projectFound = embedProject
    ...
    ?.find((project, i) => {
      ...
      if(!projectHasAppLocale(project)){
        return false;
      }

      //TODO P1_remove this logic
      if (
        [
      ...
      if (
        ...
        sendObserverLogs(
          {
            ...
            properties: {
              terminationRule: {
                nSession: allProjectStatus?.[project.id]?.terminationStatus?.nSession
              },
              ...extractProjectDataForObserver(project)
            },
            level: ObserverLogLevel.DEBUG,
        ...
        if (!allProjectStatus?.[project?.id]?.isCompleted) {
          removeProjectFromStatusArray({id: project?.id, saveState: false}, traceId);
        }
        return false;
      } else if (
        (
        ...
        if (
          ...
          terminationRule = {
            nDismissedByUser: allProjectStatus?.[project.id]?.terminationStatus?.nDismissedByUser
          }
        } else if (
          allProjectStatus?.[project.id]?.initialTerminationStatus?.perApp !== -1 &&
          allProjectStatus?.[project.id]?.terminationStatus?.perApp >= allProjectStatus?.[project.id]?.initialTerminationStatus?.perApp
          ...
          terminationRule = {
            perApp: allProjectStatus?.[project.id]?.terminationStatus?.perApp
          }
        } else {
          terminationRule = {
            untilAllFlowsAreCompleted: allProjectStatus?.[project.id]?.terminationStatus?.untilAllFlowsAreCompleted
          }
        }
      ...
      switch (true) {
        ...
        case project.isAFlowMenu: {
          ...
          let assistIdentifiers = project?.isWebView ? [
          ...
          let found = checkIfIdentifierMatch(
          ...
          sendObserverLogs(
            {
              ...
              properties: {
                ...extractProjectDataForObserver(project),
                assistIdentifiers,
                didProjectIdentifiersMatch: found,
                shouldShowProject: (
                  (
          ...
          if (found && (((!allProjectStatus[project?.id]?.isShown ||
                  !allProjectStatus[project?.id]?.isCompleted ||
                  project?.isIconEnabled) && checkToShowProject(project?.id)) ||
              searchOnlyFlowMenu)
          ){
            return project;
            }
        ...
        case !project.isWebView: {
          ...
          let assistIdentifiers = project.isAnchoredElement ? [...project.assist.nativeIdentifiers, project.targetId] : project.assist.nativeIdentifiers
        ...
        case project.isWebView: {
          ...
          let assistIdentifiers = project.isAnchoredElement ? [...project.assist.webIdentifiers, project.targetId] : project.assist.webIdentifiers;
...
function runDetectionLogicForSteps({lastStep}, webIdentifiers, traceId) {
  ...
  } else if (lastStep?.toShow && !lastStep?.completed) {
  ...
  activeProject.flows[0].pages.find((page) => {
    const didPageIdentifiersMatch = checkIfIdentifierMatch(
    ...
    sendObserverLogs(
      {
        ...
        properties: {
          ...extractProjectDataForObserver(activeProject),
          currentStepIds,
          pageIdentifiers: page.nativeIdentifiers,
          didPageIdentifiersMatch
        },
        level: ObserverLogLevel.DEBUG,
      },
      traceId
...
function hasAnchorNode(assistType) {
  return ["TOOLTIP",
    "HIGHLIGHT_WITH_DESC",
    "BEACON",
    "FINGER_RIPPLE",
...
function getAnchorNodeInfo(
  ...
  if (!hasAnchorNode(assistType)) return null;
  ...
  if (node["is_webview"]) {
    ...
    return {
      ...
      scrollingParentInfo:  {
        uuid: anchorNode?.uuid || null,
      },
    };
  }

  const scrollableParents = getScrollableParentNodes(assistType, node,traceId);
  const scrollingParentInfoOld = scrollableParents.find((scrollableParent) => {
    return (scrollableParent?.isAppBarLayout || scrollableParent?.is_vertically_scrollable)
  })
...
function getScrollToViewScript(assistType, anchorNode, targetIdentifier) {
  ...
  let querySelectorAllStatement =
    createQuerySelectorAllStatement(targetIdentifier,"wfx_scrollElement");
  // using IIFE to not pollute the global variables of the customers
  // using top < 50 for threshold to avoid headers which is common
  // using threshold of 10% if the screen height from bottom to avoid footer which is common
  // scrollIntoViewIfNeeded fails for partial x-axis visibility,
  // so we check position  use scrollIntoView({ block: "center", inline: "center" }) first to cover all cases.
  return `
  (function(){
  try{
  ...
  if(wfx_scrollElement){
      ...
      screenHeight = screenHeight - (screenHeight * ${webviewScrollThresholds.HEIGHT});
      screenWidth = screenWidth - (screenWidth * ${webviewScrollThresholds.WIDTH});
      if(position.bottom > screenHeight
       || position.top < ${webviewScrollThresholds.TOP}
        || position.left < ${webviewScrollThresholds.LEFT} || 
...
function shouldAutoScroll2D(assistType, node, nodeBounds, parentNodes) {
  ...
  const screenWidth = flattenedHierarchy?.ROOT_NODE?.screen_width ?? -1;
  ...
  const insideBounds = parentNodes.every((parentNode)=>{
    if(parentNode?.isAppBarLayout){
      return true;
    }
    const {top:parentTop,left:parentLeft,right:parentRight,bottom:parentBottom} = (parentNode?.bounds || {})
    return (top >= parentTop && bottom <= parentBottom && left >= parentLeft && right <= parentRight);
  })

  // there might be some cases the parent itself outside the screen bounds
  if(insideBounds){
    // screenHeight - (screenHeight * 0.1) - to resolve SUCC-34548
...
function getScrollableParentNodesInfo(anchorNode, parentNodes=[]) {
  ...
  const isVerticallyScrollable = canViewVerticallyScroll(anchorNode);
  const isHorizontallyScrollable = canViewHorizontallyScroll(anchorNode);
  if (isVerticallyScrollable || isHorizontallyScrollable) {
    parentNodes.push({
      ...
      bounds:anchorNode?.bounds
    });
  }
  const parentUUID = anchorNode["parent_uuid"];
  // The anchor node null handler fails when invalid data is provided, resulting in an undefined ID map
  if(!parentUUID){
    return parentNodes;
  }
...
function updateAssistInteraction(res, traceId) {
  ...
  if (actionCallback.carouselSlideId) {
    sendAnalyticEvent(ANALYTICS_EVENTS.ELEMENT_ACTION, {
      ...
      carouselPreviousSlideId:
        (!actionCallback.carouselPreviousSlideId || actionCallback.carouselPreviousSlideId === "null")
          ? null
          : actionCallback.carouselPreviousSlideId,
      carouselDirection:
        (!actionCallback.carouselDirection || actionCallback.carouselDirection === "null")
          ? null
    ...
    if(["TOOLTIP","HIGHLIGHT_WITH_DESC"].includes(activeProject?.projectType) &&
        actionCallback.actionType === "overlayClicked"){
      const projectDetails = embedProject.concat(sortedProjectArray).find((project)=>{
        return project.id === activeProject?.id;
      })
      if(projectDetails){
        const dismissableOnOverlayClick = !!(projectDetails?.assist?.instruction?.assistInfo?.layoutInfo?.dismissAction?.outsideDismiss)
        if(dismissableOnOverlayClick){
          sendMessageToNative({
            command: SENDING_COMMANDS.NO_CONTEXT_FOUND,
          }, traceId);
        }
    ...
    sendAnalyticEvent(ANALYTICS_EVENTS.ELEMENT_ACTION, {
      ...
      actionEventValue: actionCallback?.actionType
    }, traceId);
  ...
  // commenting since nDismissedByUser is not used for assists
  // if (
  //   actionCallback.close &&
  //   !actionCallback.optIn &&
  //   currentlyRunningFlow.id !== activeProject.id &&
  ...
  if (
    ...
    const lastStep = currentStaticFlow.stepsStatus[currentStaticFlow.stepsStatus.length - 1];
    flowShown = false;

    if (lastStep.completionStep) {
      sendAnalyticEvent(ANALYTICS_EVENTS.FLOW_SUCCESS, undefined, traceId);
      if (currentlyRunningFlow.id !== "") {
        const currentRunningFlow = completedProjectsInFlow[currentlyRunningFlow.id];
...
function createIfElseStatement(
  ...
  const querySelectorAllStatement = createQuerySelectorAllStatement(webIdentifier,"wfx_assist_ele");
  let condition = `wfx_assist_ele`;
  if (
      webIdentifier["innerText"] && typeof webIdentifier["innerText"]["ang"] === "string"
  ) {
    const expectedText = webIdentifier["innerText"]["ang"].replace(/"/g, '\\"');
    condition += ` && wfx_assist_ele.innerText === "${expectedText}"`;
  }
  return `
    ${querySelectorAllStatement}
    if (${condition}) {
...
function createQuerySelectorAllStatement(webIdentifier,elementVariable) {
  let queryStatement = '';

  if (webIdentifier.customSelector) {
    webIdentifier.customSelector.split("<=").forEach(selector => {
      if (queryStatement !== '') {
        queryStatement = "?.shadowRoot" + queryStatement;
      }
      queryStatement = `.querySelector('${selector}')` + queryStatement;
    })
  ...
  if(features?.webviewFilterInvisibleElements === true){
    return  `
        let ${elementVariable} = document.querySelectorAll(${JSON.stringify(cssSelector)});
       ...
       ${elementVariable} = Array.from(${elementVariable}).filter(isElementVisible)[${index}]
  `
  }
  ...
  try {
    if (!jsSdkInitialized) {
      ...
      if (!isSelfHosted) {
        sendObserverLogs(
          {
            event_name: OBSERVER_EVENTS.NETWORK.STARTED_POLLING_JS_METADATA,
...
function handleProjectsMissingInAllProjectStatus() {
  try {
    sortedProjectArray?.forEach((project) => {
      let projectStatusId = project.id;
      let initialCompletedMap = {};
      if (project.connectedProjects !== undefined) {
        computeCompletedProjectsInFlow(project, projectStatusId, completedProjectsInFlow, initialCompletedMap);
      }

      let initialTerminationStatus = {
        perSession: project?.initialTerminationStatus?.perSession || -1,
        perApp: project?.initialTerminationStatus?.perApp || -1,
        untilAllFlowsAreCompleted: project?.initialTerminationStatus?.untilAllFlowsAreCompleted || false,
        nSession: project?.initialTerminationStatus?.nSession || -1,
        nDismissedByUser: project?.initialTerminationStatus?.nDismissedByUser || -1,
      }

      let terminationStatus = {
        perSession: allProjectStatus?.[projectStatusId]?.terminationStatus?.perSession || project?.terminationStatus?.perSession || 0,
        perApp: allProjectStatus?.[projectStatusId]?.terminationStatus?.perApp || project?.terminationStatus?.perApp || 0,
        untilAllFlowsAreCompleted: allProjectStatus?.[projectStatusId]?.terminationStatus?.untilAllFlowsAreCompleted || project?.terminationStatus?.untilAllFlowsAreCompleted || false,
        nSession: allProjectStatus?.[projectStatusId]?.terminationStatus?.nSession || project?.terminationStatus?.nSession || 0,
        nDismissedByUser: allProjectStatus?.[projectStatusId]?.terminationStatus?.nDismissedByUser || project?.terminationStatus?.nDismissedByUser || 0,
      }
      ...
      allProjectStatus[projectStatusId] = {
        ...
        shownOnce: false,
        isShown: false,
        isCompleted: (initialTerminationStatus.nSession !== -1 && initialTerminationStatus?.nSession === terminationStatus?.nSession),
        isFlowTriggered: false,
        isTerminated: false,
        triggerCase: project?.triggerCase,
...
async function fetchConfig({
  ...
  try {
    if (!isSelfHosted) {
      const sanitizedHeadersForObserver = {...headers};
      const sanitizedWfxProfileForObserver = {...wfxProfileBody};

      delete sanitizedHeadersForObserver["x-jiny-client-id"];
      delete sanitizedWfxProfileForObserver["id"];

      const entId = jsMetadata.entId || sdkStateService.entId;
      const appId = jsMetadata.appId || sdkStateService.wfAppId;
      let configFetchURL = `${configBaseUrl}odin/api/v1/config/fetch`;

      if (entId && appId && appVersionName && jsVersionNumber) {
        configFetchURL = `${configBaseUrl}odin/api/v1/ents/${entId}/apps/${appId}/app-versions/${appVersionName}/js-versions/${jsVersionNumber}/config/fetch`;
      }
      ...
      sendObserverLogs(
        {
          ...
          properties: loggerConfig?.log_types?.includes(ObserverLogLevel.DEBUG)
            ? {
                url: configFetchURL,
                source,
                headers: sanitizedHeadersForObserver,
                requestBody: {...requestBody, wfxProfile: sanitizedWfxProfileForObserver},
              }
            : {},
          level: ObserverLogLevel.DEBUG,
        },
      ...
      let response;
      try {
        response = await fetch(configFetchURL, {
          method: "POST",
          headers: headers,
          body: dataString,
        });
      ...
      sendObserverLogs({
        ...
        properties: {
          ...
          ...(loggerConfig?.log_types?.includes(ObserverLogLevel.DEBUG) ? {
            featureFlagList: config?.feature ? Object.keys(config?.feature) : ["no feature flags"],
            metaConfigList: config?.metaConfig ? Object.keys(config?.metaConfig) : ["no meta config"],
            wfxProfileProjectStatus: config?.wfxProfile?.properties?.allProjectStatus || {},
            WfxCompletedProjectsInFlow: config?.wfxProfile?.properties?.completedFlowProjects || {},
            savedStateProjectStatus: savedUserState?.projectsStatus || {},
            savedCompletedProjectsInFlow: savedState?.projectsCompletionStats || {}
          } : {}),
    ...
    switch (status) {
      case 200: {
        if (config.configStatus === 200) {
          ...
          if (config?.wfxProfile?.status === 200 && config?.wfxProfile?.properties) {
            ...
            if (!isEmptyObject(config?.wfxProfile?.properties?.allProjectStatus)) {
            ...
            allProjectStatus = wfxProfileProperties?.allProjectStatus || savedUserState?.allProjectStatus || {};
            completedProjectsInFlow = wfxProfileProperties?.completedFlowProjects;
            selectedLanguage = appLocale || savedUserState.language || wfxProfileProperties?.selectedLanguage;
      ...
      default: {
        console.error(`UNABLE_TO_FETCH_CONFIG_THROWING_${status}`);
      }
    }
  } catch (error) {
...
function updateUserProfile(traceId) {
  try {
    ...
    userProfileService.debouncedEnqueueUpdate(userId, traceId);
  } catch (error) {
...
function organizeProjectArray(
  ...
  if (isEmbed) {
    ...
    if (wfxProfile && wfxProfile.properties) {
      ...
      selectedLanguage = appLocale || selectedLanguage || wfxProf?.selectedLanguage ;
...
// // Method to print persisted logs
// function printPersistedLogs() {
//   persistedLogs.forEach(entry => {
//     const prefix = `[${entry.timestamp}] [${entry.type}]`;
//     originalConsoleLog(prefix, ...entry.args);
//   });
// }
...
class ApiService {
  ...
  callApi = async (request, requestOptions) => {
    return new Promise((resolve, reject) => {
      ...
      const api = async () => {
        try {
          const headers = await globalOptions.getHeaders();
...
var surveyAnalyticsService = (() => {
  ...
  async function getHeaders() {
    try{
      if(!(jsMetadata?.analyticsMetadata?.surveyAuthToken)){
        await fetchAndSetEnterpriseId(generateUUID())
      }
      return {
      ...
      console.error("getHeaders(): error while returning the headers", error);
      return ({})
    }
  ...
  function getApiPayload(data = {}) {
    try {
      ...
      const getProperties = (info) => {
        ...
        return {
          wf_app_id: jsMetadata?.appId || wfAppId,
          wf_org_id: jsMetadata?.orgId || wfOrgId,
          wfe_app_id: jsMetadata?.appId || wfAppId,
          wfe_survey_question_id,
          wfe_survey_question,
          wfu_survey_answer,
  ...
  async function sendEvent(data = {}) {
    ...
    if (!jsMetadata) {
      jsMetadata = await fetchAndSetEnterpriseId(generateUUID());
    }
    const {surveyEventsUrl} = jsMetadata?.analyticsMetadata || {};
    const url = surveyEventsUrl || `${appObserverBaseUrl}service/survey/edith/api/v1/analytics/source/${enterpriseId}`;
...
const eventCollectorService = {
    init(maxBatchSize = 5, batchWaitTime = toMilliSeconds(10)) {
        ...
        this.eventRequestQueueIds = new Set();
        this.uploadingEventIds = new Set();
    ...
    getEventId(event){
        return event?.properties?.wf_event_id;
    },
    pushEventsToQueue(events=[]){
        if(!events || !events.length) return;
        events.forEach(event => {
            const eventId = this.getEventId(event);
            if(eventId && !this.eventRequestQueueIds.has(eventId) && !this.uploadingEventIds.has(eventId)) {
                this.eventRequestQueueIds.add(eventId);
                this.eventRequestQueue.push(event);
            }
        })
    },
    unShiftEventsToQueue(events=[]){
        if(!events || !events.length) return;
        let eventsToUnshift = [];
        events.forEach(event => {
            const eventId = this.getEventId(event);
            if(eventId && !this.eventRequestQueueIds.has(eventId)  && !this.uploadingEventIds.has(eventId)){
                this.eventRequestQueueIds.add(eventId);
                eventsToUnshift.push(event);
            }
        })
        this.eventRequestQueue.unshift(...eventsToUnshift);
    },

    clearEventsQueue(){
      this.eventRequestQueue = []
      this.eventRequestQueueIds.clear();
    },

    spliceEventsFromQueue(start,end){
      const batch = this.eventRequestQueue.splice(start,end);
        batch.forEach(event => {
            const eventId = this.getEventId(event);
            if(eventId){
                this.eventRequestQueueIds.delete(eventId);
            }
        })
        return batch;
    },

    addToUploadingEventIds(events=[]){
        if(!events || !events.length) return;
        events.forEach(event => {
            const eventId = this.getEventId(event);
            if(eventId){
                this.uploadingEventIds.add(eventId);
            }
        })
    },

    removeFromUploadingEventIds(events=[]){
        if(!events || !events.length) return;
        events.forEach(event => {
            const eventId = this.getEventId(event);
            if(eventId){
                this.uploadingEventIds.delete(eventId);
            }
        })
    },

    async enqueueEventRequests(eventRequests = [], traceId) {
        if (isSelfHosted || inPreviewMode) return;
        ...
        this.pushEventsToQueue(eventsPendingToBeAdded)
    ...
    async uploadNextBatch(traceId) {
        if (this.isUploading || !this.eventRequestQueue.length) {
            return;
        }
        this.isUploading = true;
        let batch;
        try {
            // decoupling the async function to avoid concurrency issues with multiple calls to uploadNextBatch before the first one sets isUploading to true
            ...
            batch = this.spliceEventsFromQueue(0,this.maxBatchSize);
            this.addUploadMetadataToEventPayload(batch);
            this.addToUploadingEventIds(batch);
            await this.uploadEvents(batch);
            this.removeFromUploadingEventIds(batch)
        } catch (error) {
            console.error(`[${traceId}]Error uploading events`, error);
            if(batch){
                this.removeFromUploadingEventIds(batch)
                this.unShiftEventsToQueue(batch);
            }
        } finally {
            this.cacheEventQueue()
            this.isUploading = false;
    ...
    getEventCollectorUrl() {
        return jsMetadata?.analyticsMetadata?.eventsUrl || `${appObserverBaseUrl}service/event/collect/v2/${enterpriseId}`;
    },
    ...
    async canMakeApiCall() {
        ...
        const hasAppId = typeof wfAppId === 'string' && wfAppId.trim() !== '';
        const hasOrgId = typeof wfOrgId === 'string' && wfOrgId.trim() !== '';

        if (!hasEnterpriseId) console.warn("event upload failed: enterpriseId is missing or invalid");
        if (!hasAppId) console.warn("event upload failed: appId is missing or invalid");
        if (!hasOrgId) console.warn("event upload failed: orgId is missing or invalid");

        // Check network connectivity
        const online = await this.isDeviceOnline();
        console.debug("eventCollectorService.uploadEvents(): online status ", online);
        if (online && (!hasEnterpriseId || !hasAppId || !hasOrgId)) {
    ...
    buildAnalyticsEventObject(eventRequest, traceId) {
        ...
        let appId = jsMetadata?.appId || wfAppId;
        let orgId = jsMetadata?.orgId || wfOrgId;
            ...
            properties: {
              ...
              ...(activeStep?.id ? {
                activeStep: {
                  step_id: activeStep?.id,
                  step_name: activeStep?.step?.assist.name,
                  type: activeStep?.step?.assistType,
                }
              } : {})
            },
            level: ObserverLogLevel.DEBUG,
...
const LOG_FIELD_TYPE_DEFAULT = "DEFAULT";

const observerLogService = {
  init(maxBatchSize = 40, batchWaitTime = toMilliSeconds(10)) {
    this.maxBatchSize = maxBatchSize;
    this.batchWaitTime = batchWaitTime;
  ...
  generateLogBody({
    ...
    return {
      ...
      on_id: `${serialNumber || 0}`,
      message_id: generateUUID(),
      class_name: 'leap_sdk',
      method_name: category ? `eventCategory: ${category}` : LOG_API_REQUEST_NULL_STRING_VALUE,
      log_data: {
        ...
        "string.timestamp": `${timestamp || Date.now()}`,
        "string.js_version": `${jsVersionNumber || 0}`,
  ...
  sendObserverLog({
    ...
    try {
      const shouldSendLogs = forceSendObserverLogs || loggerConfig?.log_types?.includes(level);
      if (shouldSendLogs) {
        const logRequest = {
          message: eventName,
          category,
        ...
        // console.log('NEW_OBSERVER_LOG', logRequest);
        this.queueLogRequests([logRequest], traceId);

        // for recording and creating an object with detailed report on observer log sizes
        // analyzeObserverLogPayloadSize(logRequest);
      }
    } catch (error) {
...
function analyzeObserverLogPayloadSize(logRequest) {
  const logSize = JSON.stringify(logRequest).length;
  ...
  if (observerDataSize.keys.hasOwnProperty(logRequest.message)) {
    let data = observerDataSize.keys[logRequest.message];
    data.numberOfEvents++;
...
const userProfileService = {
  ...
  debouncedEnqueueUpdate(userId, traceId) {
    // Cancel existing debounce if it's for a different user
    if (this.debounceTimerId && this.debouncedUserId !== userId) {
      clearTimeout(this.debounceTimerId);
      this.debounceTimerId = null;
      // Immediately process the previous user's update
      if (this.debouncedUserId) {
        this.enqueueUpdate(this.debouncedUserId, traceId);
      }
      this.debouncedUserId = null;
    }

    // Get debounce delay
    const debounceDelay = jsMetadata?.profile?.delay || sdkStateService?.profileConfig?.delay || 0;
    ...
    // Store the current userId being debounced
    ...
    this.debounceTimerId = setTimeout(() => {
      this.debounceTimerId = null;
      this.debouncedUserId = null;
      this.enqueueUpdate(userId, traceId);
    }, debounceDelay * 1000);
  },
  ...
  async startUserUpdate(userId, traceId) {
    ...
    const requestBody = !isEmptyObject(savedUserState.projectsStatus) ?
      {
        id: userId,
        properties: {
  ...
  async tryUpload(userId, requestBody, traceId) {
    try {
      ...
      const entId = jsMetadata.entId || sdkStateService.entId;
      const appId = jsMetadata.appId || sdkStateService.wfAppId;
      let profileURL = `${configBaseUrl}odin/api/v1/profile`

      if (entId && appId && appVersionName && jsVersionNumber) {
        profileURL = `${configBaseUrl}odin/api/v1/ents/${entId}/apps/${appId}/app-versions/${appVersionName}/js-versions/${jsVersionNumber}/profile`;
      }
      ...
      sendObserverLogs(
        {
          ...
          userId,
          // level: ObserverLogLevel.ERROR, -> moved to DEBUG temporarily since profile api is blocked
          level: ObserverLogLevel.DEBUG,
        },
        traceId
      );
  ...
  async flushAll(traceId) {
    // Clear any pending debounced updates before flushing all
    if (this.debounceTimerId && this.debouncedUserId) {
      clearTimeout(this.debounceTimerId);
      this.debounceTimerId = null;
      // Immediately process the debounced user
      this.userIds.enqueue({userId: this.debouncedUserId, traceId});
      this.startUserUpdateIfNeeded(this.debouncedUserId, traceId);
      this.debouncedUserId = null;
    }
...
async function startPollingJSMetadata() {
  const isDeviceOnline = await networkService.isReallyOnline();
  if (isDeviceOnline) {
    await setEnterpriseId();
    jsMetadataPollFrequency = jsMetadata?.pollFrequency?.jsMetadata || jsMetadataPollFrequency;
  }

  setTimeout(async () => {
    await startPollingJSMetadata()
  }, jsMetadataPollFrequency * 1000);
}
...
async function setEnterpriseId(){
    ...
    if (enterpriseId) {
      ...
      sdkStateService.profileConfig = jsMetadata?.profile;
...
async function fetchAndSetEnterpriseId(traceId) {
    ...
    if (jsMetadataResponse !== null) {
      ...
      wfOrgId = jsMetadataResponse?.orgId;
...
async function fetchObserverMetadata(traceId) {
    ...
    try {
        console.log("fetchObserverMetadata(): Fetching observer metadata", {baseUrl, appApiKey});
        sendObserverLogs(
          {
            category: OBSERVER_EVENT_CATEGORIES.NETWORK,
